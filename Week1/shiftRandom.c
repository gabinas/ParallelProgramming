...
void spmd(){
	...
	int randNum = rand()%50; /*Generate the random number that will be sent to another processor */
	int r;
	// 1.2
	...
	// 2.1
	bsp_put((bsp_pid() - 1) % bsp_nprocs(), &randNum, &r, 0, sizeof(randNum)); /*This is where the shifting happens, by sending s to the processor to the left */
	// 2.2
